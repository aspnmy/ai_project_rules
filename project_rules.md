...
#### Git相关规则  
- 详细规则请参考次级规则文件：${PG_sub_rulesfiles}中的git_commit_rules.txt
- 版本控制和代码管理相关规则
...
### 🔴 高优先级规范（文件隔离规范规则）
* **文件隔离查找规则**：
  - 系统只递归向下查找PG_RuleFileName文件同级目录下方的内容🔴
  - PG_RuleFileName文件同级目录被视为根目录🔴
  - 无论工作区中是否还存在其他项目，都只向下查找，不向上查找🔴
  - 此规则确保项目文件查找的隔离性和明确性，避免跨项目干扰🔴
...
### Unity游戏Mod开发规范
    * **次级规范文件**：详细的Unity游戏Mod开发相关规则已移至 `rules\unity_mod_rules.txt` 文件中
    * **运行环境要求**：
...
### 编写JavaScript代码，应该遵循Google JavaScript风格指南：
    * **次级规范文件**：详细的JavaScript编码规则已移至 `rules\js_coding_rules.txt` 文件中
    * **语言规范**：
...
### 编写HTML/CSS代码，应该遵循Google HTML/CSS风格指南：
    * **次级规范文件**：详细的HTML/CSS编码规则已移至 `rules\html_css_coding_rules.txt` 文件中
    * **HTML样式规则**：
      * **文档类型**：使用HTML5文档类型声明
...
### Markdown文档写作规范：
    * **次级规范文件**：详细的Markdown写作规则已移至 `rules\markdown_rules.txt` 文件中
    * **全局规范**：
      * 文件必须使用.md作为后缀（小写字母）
      * 普通文本换行，使用行末尾2空格触发
    * **标题结构格式**：
...
### 编写Java代码，应该遵循Google Java风格指南：
    * **次级规范文件**：详细的Java编码规则已移至 `rules\java_coding_rules.txt` 文件中
    * **源文件基础**：
...
### 日志记录规范及最佳实践：
    * **次级规范文件**：详细的日志相关规则已移至 `rules\logs_rules.txt` 文件中
    * **日志基础概念**：
...
### 次级规则文件查询机制
* **查询顺序**：
  - 查询规则规范时，应首先查询次级规则文件（位于rules\目录下的.txt文件）🔴
  - 如果次级规则文件中不存在该规则，则再查询${PG_RuleFileName}主规则文件🔴
  - 此机制用于提高查询效率，将不同类型的规则分类存储到专门的次级规则文件中
